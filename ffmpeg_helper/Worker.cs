    ...
    public sealed class Worker
    {
        ...
        bool Single(Job job)
        {
            ...
            switch (a.type.ToLower())
            {
                case "cut":
                    index++;
                    string output_name = string.IsNullOrEmpty(a.output) ? $"temp_{index}{ext}" : a.output;
        ...
        bool Multiple(Job job)
        {
            ...
            foreach(var i in a)
            {
                ...
                switch (i.type.ToLower())
                {
                    case "cut":
                        index++;
                        string output_name = string.IsNullOrEmpty(i.output) ? $"temp_{index}.{ext}" : i.output;
        ...
        bool Cut(Action target, int _index, string output)
        {
            ...
            string arg = $"-ss {start} -to {realEnd} -i {data.header.filename} -codec:v copy -acodec copy \"{data.header.temp}/{output}\"";
        ...
        void Merge(Action target)
        {
            ...
            string arg = $"-f concat -safe 0 -i c.txt -c copy -y \"{data.header.output}\"";
            Console.WriteLine($"arg: {arg}");
            ProcessStartInfo info = new ProcessStartInfo("ffmpeg.exe", arg);
            info.UseShellExecute = false;
            Process proc = Process.Start(info);
